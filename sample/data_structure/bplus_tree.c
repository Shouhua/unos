...
node *findLeaf(node *const root, int key, bool verbose)
{
    ...
    while (!c->is_leaf)
    {
        ...
        if (verbose)
            printf("%d ->\n", i);
        c = (node *)c->pointers[i]; // TODO pointers里面到底放的什么？record or node？还是说叶子节点和内部节点不同？
    }
    if (verbose)
    {
        printf("Leaf [");
...
node *makeNode(void)
{
    ...
    new_node->keys = malloc((order - 1) * sizeof(int)); // 每个node最多order-1个key
    if (new_node->keys == NULL)
    {
        perror("New node keys array.");
        exit(EXIT_FAILURE);
    }
    new_node->pointers = malloc(order * sizeof(void *)); // 指向record
    ...
    new_node->num_keys = 0; // 记录当前node的key个数，因为keys是链表
    new_node->parent = NULL;
    new_node->next = NULL; // 叶子节点指向像一个node
    return new_node;
}
...
node *insertIntoLeafAfterSplitting(node *root, node *leaf, int key, record *pointer)
{
    ...
    temp_keys = malloc(order * sizeof(int)); // 超过了标准1个
    if (temp_keys == NULL)
    {
    ...
    insertion_index = 0;
    // 找到新key在node中的位置index
    while (insertion_index < order - 1 && leaf->keys[insertion_index] < key)
        insertion_index++;

    // 将leafnode中的keys和新key全部移到temp keys中
    ...
    split = cut(order - 1); // 找到分裂点m/2th, 比如m=4，分裂点是2，第三个元素

    // 将分裂点前的keys移到leaf node中
    for (i = 0; i < split; i++)
    {
    ...
    // NOTE： 双向指针，leaf node中最后一个元素指向下一个leaf node，而且是双向指向
                                   ...
                                   int key, node *right)
{
    ...
    for (i = 0, j = 0; i < old_node->num_keys + 1; i++, j++) // 这里temp_pointers新建超过标准1个，比如m=4，template_pointers个数为5
    {
        if (j == left_index + 1) // left_index的pointers空出来，后面填充
            j++;
...
node *insertIntoParent(node *root, node *left, int key, node *right)
{
    ...
    if (parent == NULL) // 如果没有root，这是推举出root, 最初情况
...
{
    node *root = makeNode();
    root->keys[0] = key; // right中的第一个
    root->pointers[0] = left; // TODO 内部节点中pointers只有指向node，没有records？
...
node *startNewTree(int key, record *pointer)
{
    node *root = makeLeaf(); // 初始isLeaf=true
    root->keys[0] = key;
...
node *insert(node *root, int key, int value)
{
    ...
    leaf = findLeaf(root, key, false); // b plus tree从下往上寻找自平衡

    if (leaf->num_keys < order - 1)
    { // 如果叶子节点里面keys个数小于最大值order-1，填充，不然分裂
